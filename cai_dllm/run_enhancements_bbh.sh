#!/bin/bash
# =============================================================================
# run_enhancements_bbh.sh
#
# Runs 4 enhancement evaluations on BBH for LLaDA and/or Dream:
...
#
# Usage:
#   bash run_enhancements_bbh.sh 0          # both models (~6-8 hours)
#   bash run_enhancements_bbh.sh 0 llada    # LLaDA only (~3-4 hours)
#   bash run_enhancements_bbh.sh 0 dream    # Dream only (~3-4 hours)
# =============================================================================
...
run_model() {
    local MODEL=$1
    local BLOCK_FREQ=$2
    local LOG="$LOG_DIR/${MODEL}_bbh_enhancements_${DATE}.log"

    echo "================================================"
    echo " Novel Enhancements — ${MODEL} BBH"
    ...
    echo ""
    echo "================================================"
    echo " Done: ${MODEL} BBH. Log: $LOG"
    echo "================================================"

    echo ""
    echo "=== QUICK SUMMARY: ${MODEL} BBH ==="
    grep -E "RUNNING:|^|bbh|Total generation time" $LOG | \
        awk '/RUNNING:/{name=$0} /Total generation/{time=$4} /^\|bbh /{
