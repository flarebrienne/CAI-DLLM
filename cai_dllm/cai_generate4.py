...
def cai_batch_generate(
    ...
    enh = enhancement_config   # shorthand
    # Idea 3: Soft Belief Injector
    sbp = SoftBeliefInjector(model, mask_id) if (enh and enh.use_soft_belief) else None
    if sbp is not None:
        sbp.enable()
    # Idea 4: Oracle Budget (replaces TokenBudgetManager when enabled)
    use_oracle = enh is not None and enh.use_oracle_budget
    # Idea 5: Prompt KV prefix cache
    prompt_cache = enh.prompt_cache if (enh and enh.use_prompt_kvcache) else None
    prompt_cache_hit = False
    # Idea 1: Adaptive Block Sizer
    abs_sizer = AdaptiveBlockSizer(
        conservative_theta=0.70,
        aggressive_theta=0.50,
        easy_threshold=enh.abs_easy_threshold,
        pos_bonus=0.05,
    ) if (enh and enh.use_adaptive_block) else None
    ...
    for num_block in range(num_blocks):
        ...
        # ── Idea 1: Adaptive Block Sizing (safe — adjusts theta_e only) ──
        # Uses step-0 confidence to decide theta_e per block.
        # EASY blocks get aggressive theta_e -> commit faster (fewer steps).
        # HARD blocks keep conservative theta_e. Buffer layout unchanged.
        abs_theta_e_override = None
        if abs_sizer is not None and num_block > 0 and all_confidence is not None:
            abs_theta_e_override = abs_sizer.get_theta_e(
                block_idx   = num_block,
                conf        = all_confidence,
                mask_index  = mask_index,
                block_start = start_index,
                block_end   = min(end_index, max_length),
            )
        actual_block_len = block_length
        ...
        # Apply ABS theta_e override for this block if available
        if abs_theta_e_override is not None:
            block_scheduler = block_scheduler.__class__(
                theta_start = block_scheduler.theta_start,
                theta_end   = abs_theta_e_override,
                warmup_frac = block_scheduler.warmup_frac,
                shape       = block_scheduler.shape,
                total_steps = block_scheduler.total_steps,
            ) if hasattr(block_scheduler, 'theta_start') else block_scheduler
        ...
        # Idea 4: use OracleBudgetManager instead of 3-tier TokenBudgetManager
        if use_oracle and use_cg:
            budget_mgr = OracleBudgetManager(block_len=block_length)
        else:
            budget_mgr = TokenBudgetManager(block_len=block_length) if use_cg else None
        # Soft grinding detector — only kills truly wasteful steps
        # (0 tokens/step for 6 consecutive steps), high conf threshold
        grind_detector = GrindingPhaseDetector(
            efficiency_thresh=0.3,   # only kills 0-token steps
            consecutive_steps=6,     # needs 6 bad steps (stricter)
            fallback_conf=0.70,      # only force-commits very confident tokens
            min_step=20,             # don't trigger before step 20
        ) if use_cg else None
        pos_scheduler  = None
        ...
        for step in range(block_length):
            ...
            # ── Idea 3: update SBP state before forward pass ─────────
            if sbp is not None and step >= enh.sbp_start_step:
                full_pos = torch.arange(0, max_length, dtype=torch.long, device=device).expand(batch_size, -1)
                top_tokens_full = all_confidence.abs().new_zeros(batch_size, max_length, dtype=torch.long)
                # top predicted token per position from last step's logits (approx via x)
                top_tokens_full = x.clone()   # committed tokens; masked positions use mask_id
                sbp.set_state(
                    confidence=all_confidence,
                    top_tokens=top_tokens_full,
                    mask_index=mask_index,
                )
    ...
    if abs_sizer is not None:
        results["adaptive_block_summary"] = abs_sizer.summary()
