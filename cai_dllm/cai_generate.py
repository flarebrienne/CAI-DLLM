...
def cai_batch_generate(
    ...
    if mask is not None and torch.any(mask == 0.0):
        ...
        real_position = torch.cumsum(mask, dim=-1).to(device).long() - 1
    ...
    for num_block in range(num_blocks):
        ...
        for step in range(block_length):
            ...
            if conv_tracker is not None:
                ...
                remaining = mask_index.sum().item()
                if conv_tracker.is_converged(num_block, block_length, remaining):
                    if cai_config.print_log:
                        print(f"Block {num_block} early exit at step {step+1}/{block_length}")
                    step_cnt += block_length - step - 1
                    break
