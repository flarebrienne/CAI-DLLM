...
def cai_batch_generate(
    ...
    abs_sizer = AdaptiveBlockSizer(
        large_size=enh.abs_large_size,
        small_size=enh.abs_small_size,
    ...
    # OUTER LOOP: blocks
    # ─────────────────────────────────────────────────────────────────────
    # Adaptive block sizing tracks cumulative offset when block sizes vary
    _abs_offset = 0   # cumulative token offset from smaller blocks
    _abs_block_idx = 0

    for num_block in range(num_blocks):
        start_index = prompt_len + num_block * block_length - token_offset + _abs_offset
        end_index   = prompt_len + (num_block + 1) * block_length - token_offset + _abs_offset

        # ── Idea 1: Adaptive Block Sizing ──────────────────────────────
        # Decide block size BEFORE inner loop using current confidence signal.
        # On the very first block (step0 confidence not yet computed), fall back
        # to full block_length. For subsequent blocks, use the confidence signal
        # from the previous step to decide whether this block is EASY or HARD.
        if abs_sizer is not None and num_block > 0 and all_confidence is not None:
            actual_block_len = abs_sizer.decide_block_size(
                block_idx   = num_block,
                conf        = all_confidence,
            ...
            if actual_block_len != block_length:
                # Adjust end_index for this block only
                end_index = start_index + actual_block_len
                end_index = min(end_index, max_length)
        else:
        ...
        # ─────────────────────────────────────────────────────────────────
        for step in range(actual_block_len):
            ...
            cur_mask = torch.gather(mask, 1, pos).long()
